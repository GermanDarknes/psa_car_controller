...
class RemoteClient:
        ...
        if charge_info is not None and (charge_info.get('remaining_time', 0) != 0 or charge_info.get('rate', 0) != 0):
            try:
                ...
                if car and car.status.get_energy('Electric').charging.status != INPROGRESS:
                    ...
                    time.sleep(60)
                    # fre42: commented out since psa cloud sends wrong infos sometimes which causes psacc waking up
                    # endlessly until the car goes to eco mode and blocks any connection
                    # self.wakeup(vin)
    ...
    # @rate_limit(6, 60 * 20) // 6 times in 20 minutes is too often and will cause the
    #                           car (at least the Opel Corsa) to go to eco mode
    @rate_limit(4, 60 * 60)
    def wakeup(self, vin):
